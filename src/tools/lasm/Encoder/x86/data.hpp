                ...
                case Instructions::MOV: {
                    ...
                    if (std::holds_alternative<Parser::Instruction::Register>(destinationOperand))
                    {
                        ...
                        if (std::holds_alternative<Parser::Instruction::Register>(sourceOperand))
                        {
                            ...
                            if (destUseREX || srcUseREX) useREX = true;
                            if (destSetREX) rexB = true;
                            if (srcSetREX) rexR = true;

                            uint8_t destRegSize = getRegSize(destReg.reg, bits);
                            uint8_t srcRegSize = getRegSize(srcReg.reg, bits);

                            if (destRegSize != srcRegSize) throw Exception::SemanticError("Can't use 'mov' with registers of different size", -1, -1);
                            ...
                            if (usingSpecialRegDest || usingSpecialRegSrc) throw Exception::SemanticError("Can't set special register using special register", -1, -1);
                        ...
                        else if (std::holds_alternative<Parser::Immediate>(sourceOperand))
                        {
                            ...
                            switch (destReg.reg)
                            {
                                ...
                                default: throw Exception::InternalError("Unknown register", -1, -1);
                            }
                        }
                    }
